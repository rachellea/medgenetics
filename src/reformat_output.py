...
def make_output_human_readable(gene_name, df, scaler, raw_data):
    ...
    for index, row in df.iterrows():
        ...
        for column in df.columns:
            if column.startswith('Consensus_') and row[column]==1:
                consensus = column[-1] #e.g. if column is Consensus_A append only the 'A' part (i.e. -1)
    ...
    #get the domains by reversing one hot encoding
    domains = []
    for index, row in df.iterrows():
        found = False
        for column in df.columns:
            if column.startswith('Domain_') and row[column]==1:
                domains.append(column.replace('Domain_',''))
                found = True
        if not found:
            assert False, 'Found example with no domain '+str(index)
    new_df['Domain'] = domains
    
    # get the original continuous variables by performing inverse transform
    inverted_cont_vars = scaler.inverse_transform(df[['Position', 'Conservation', 'SigNoise','PSSM','RateOfEvolution']].values)
    #initialize df columns as numeric:
    new_df['Position'] = 0.0
    new_df['Conservation'] = 0.0
    new_df['SigNoise'] = 0.0
    new_df['PSSM'] = 0.0
    new_df['RateOfEvolution'] = 0.0
    #fill in df columns:
    new_df[['Position','Conservation','SigNoise','PSSM','RateOfEvolution']] = inverted_cont_vars
    ...
    assert (new_df[['Consensus','Change','Position','Domain','True_Label']].values == raw_data[['Consensus','Change','Position','Domain','True_Label']].values).all().all()
    assert (np.isclose(new_df[['Conservation','SigNoise','PSSM','RateOfEvolution']].values, raw_data[['Conservation','SigNoise','PSSM','RateOfEvolution']].values)).all().all()
    return new_df
