...
def graph_mysteryAAs_results():
    mysteryAAs = pd.read_csv('/data/rlb61/Collaborations/Landstrom/2018-11-02-Neural-Net-All-Probabilities/mysteryAAs_results_epoch_678.csv')
    mysteryAAs.sort_values(by='Position',ascending=True,inplace=True)
    ...
    mysteryAAs['Position'] = pd.to_numeric((((mysteryAAs['Position']*scale[0])+mean[0]).round(decimals=0)), downcast = 'integer')
    
    #Now get the mean predicted probability
    selected = mysteryAAs[['Position','Pred_Prob']]
    result = selected.groupby('Position')['Pred_Prob'].mean()
    result.to_csv('/data/rlb61/Collaborations/Landstrom/2018-11-02-Neural-Net-All-Probabilities/mysteryAAs_mean.csv',
    ...
    plt.savefig('mysteryAAs_pred_probs_figure.pdf')
    plt.close()

if __name__=='__main__':
    graph_mysteryAAs_results()