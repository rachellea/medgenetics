...
class MLP_ensemble(object):
    ...
    def train_test_evaluate(self):
        ...
        print("WE'RE HERREEEEEEEEEEEEEEEEEEEE")
        # for ensembling, get the list of predicted lables from best epoch
        self.best_pred = self.epoch_dict[self.best_valid_loss_epoch]
    ...
    def train(self):
        #------------------------for ensembling-----------------------------
        # initialize an empty dictionary to store list of predicted labels for each epoch
        # this epoch dict will be updated in test()
        self.epoch_dict = {}
    ...
    def test(self, chosen_dataset):
        ...
        if chosen_dataset == "Test":
            # update the epoch dictionary
            if self.num_epochs_done not in self.epoch_dict:
                self.epoch_dict[self.num_epochs_done] = []
            # append predicted labels and predicted probabilities to the list
            self.epoch_dict[self.num_epochs_done].append(selected_pred_labels)
            self.epoch_dict[self.num_epochs_done].append(selected_pred_probs)
